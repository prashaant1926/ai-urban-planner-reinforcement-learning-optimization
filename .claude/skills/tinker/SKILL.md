...
After any Tinker API call that incurs cost (forward_backward, sample, etc.), print this line for billing:

```python
# After API call, report usage for billing
print(f"SUNDIAL_BILLING:tinker,{model_name},{total_tokens},{cost_usd}")
```
...
# Report usage (estimate: ~$0.0002 per 1K tokens for Qwen3-30B)
tokens = len(datum.model_input.tokens)
cost = tokens * 0.00000022  # $0.22 per 1M tokens
print(f"SUNDIAL_BILLING:tinker,qwen3-30b,{tokens},{cost:.6f}")
```
